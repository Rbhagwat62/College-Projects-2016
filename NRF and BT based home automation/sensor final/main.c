...
teem[rc]=rx;
	...
	if(rx=='G')			  //system arm if receive G on nrf
	{seet=1;
	sensed=1;
	//USART_puts("sys ARM seet=");
	//SerialPrint(seet);
	//USART_puts("sensed");
	//SerialPrint(sensed);
		...
		if(rx=='D')						//system disarm	if receives D on nrf
		{//USART_puts("sys DRM  seet=");
		sensed=0;
		//SerialPrint(seet);
		//USART_puts("sensed");
		//SerialPrint(sensed); 
...
//	USART_puts("\n the main loop");
	...
	for(i=1;i<10;i=i+2)
		...
		//USART_puts("delay start");
		delay_ms(7000);			
		//USART_puts("delay end");
		write_byt_rxd();
	while(1)
	{		 // USART_puts("\nwhile loop");
			if(sw1==0)
